                    ...
                    {dataAboutPage.map((data) => (
                        <div key={data.id} className='relative py-6 pl-8 sm:pl-32 group'>
                            ...
                            <div className='flex flex-col sm:flex-row items-start mb-1 
                                ...
                                <time className='sm:absolute left-0 translate-y-0.5 inline-flex items-center justify-center text-xs font-semibold uppercase w-20 h-6 mb-3 sm:mb-0 text-yellow-900 bg-yellow-100 rounded-full'>{data.date}</time>
                                <p className='text-xl font-bold text-white-400'>{data.subtitle}</p>
                            </div>
                            <p className='text-slate-300 max-w-md'>{data.description}</p>
                        </div>
                    ))}
                </div>
        </div>
